<template>
  <el-row type="flex" justify="center">
    <el-col :span="23">
      <el-row type="flex" v-if="printTF">
        <el-col>
          <font color="red">没有安装打印控件</font>
        </el-col>
      </el-row>
      <!--表单  -->
      <el-row type="flex">
      ...
      <el-row type="flex">
        <el-col>
          <tableXiala
          :height = "610"
          :columnProps="columnProps" :tableData="tableData" @handleSelectionChange="handleSelectionChange" @handlePrint="handlePrint"></tableXiala>
        </el-col>
      </el-row>
    </el-col>
    <el-dialog
...
<script>
// import '../../../static/vendor/LodopFuncs'
...
import {RecordSendShow} from '../../api/yunshuguanli/fcjl'
...
export default {
  ...
  data () {
    return {
      ...
      mozhu: {
        data: [
          {props: 'Transport_Curtime', label: '日期', placeholder: '选择日期', select: '', input: '', date: true, content: [], guest: false},
          {props: 'Transport_Platenumber', label: '车牌号', placeholder: '车牌号', select: '', input: true, date: '', content: [], guest: false},
          {props: 'Transport_Endnet', label: '目的站', placeholder: '目的站', select: true, input: '', date: '', content: [], guest: true}
        ],
        model: {
          Transport_Curtime: '',
  ...
  created () {
    ...
    // this.mozhu.data[2].content[0] = {value: 11}
  ...
  methods: {
    ...
    async showData () {
      let show = await RecordSendShow({searchData: this.searchData})
