...
import {ReceiptcountShow} from '../../api/baobiaofenxi/hdsxtj.js'
...
export default {
    ...
    return {
      ...
      column: {
        columnProps: [
          ...
          { props: 'Receipt_Time', label: '回单签收时间' }
        ],
        columnTF: {
          'Order_BillID': true,
          'Order_BaseTime': true,
  ...
  methods: {
    ...
    async show () {
      let gcs = await ReceiptcountShow({currentPage: this.currentPage, Order_Billnet: this.nodeData, pageCount: this.pageCount, searchData: this.momo.model})
      this.tableData = gcs.data.info
      this.total = gcs.data.count
    },
    async customSetting () {
      let customData = await SettlementCustom({Cus_Type: 'hdsxtj'})
    ...
    async customGo () {
      ...
      let ct = await SettlementCusadd({Cus_Name: ctData, Cus_Type: 'hdsxtj'})
      if (ct.data.result === '1') {
        this.$message({
          showClose: true,
    ...
    getSummaries (param) {
      ...
      for (let zz in this.column.columnProps) {
        if (!this.column.columnProps[zz].computed) {
          sums[zz] = ''
        }
      }
      return sums
