...
import {AbnormalcountShow} from '../../api/baobiaofenxi/ycdjjk.js'
...
export default {
    ...
    return {
      ...
      column: {
        columnProps: [
          ...
          { props: 'Order_End', label: '目的站', computed: '' },
          ...
          { props: 'Order_BaseTime', label: '日期' },
          { props: 'Order_Abnormal', label: '异常类型(修改/挂失/冻结)' },
  ...
  methods: {
    ...
    async show () {
      let gcs = await AbnormalcountShow({currentPage: this.currentPage, Order_Billnet: this.nodeData, pageCount: this.pageCount, searchData: this.momo.model})
      this.tableData = gcs.data.info
      this.total = gcs.data.count
    },
    async customSetting () {
      let customData = await SettlementCustom({Cus_Type: 'ycdjjk'})
    ...
    async customGo () {
      ...
      let ct = await SettlementCusadd({Cus_Name: ctData, Cus_Type: 'ycdjjk'})
      if (ct.data.result === '1') {
        this.$message({
          showClose: true,
