...
import {RemainingShow} from '../../api/baobiaofenxi/shyltj.js'
...
export default {
  ...
  data () {
    return {
      ...
      column: {
        columnProps: [
          { props: 'Order_BillID', label: '运单号', computed: false },
          { props: 'Order_BaseTime', label: '开单日期', computed: false },
          { props: 'Order_Send', label: '始发站', computed: false },
          { props: 'Order_End', label: '目的站', computed: false },
          { props: 'Order_Receive', label: '收货人', computed: false },
          { props: 'Receive_LinkTel', label: '收货电话', computed: false },
          { props: 'Order_Consignor', label: '发货人', computed: false },
          { props: 'Order_ConTel', label: '发货电话', computed: false },
          { props: 'Order_Goods', label: '货物名称', computed: false },
          { props: 'Order_Package', label: '包装', computed: false },
          { props: 'Order_PayType', label: '运费方式', computed: false },
          { props: 'Order_PaymentType', label: '货款方式', computed: false },
          { props: 'Order_PickType', label: '提货方式', computed: false },
          { props: 'Cost_Startcost', label: '始发中转费', computed: true },
  ...
  methods: {
    ...
    async show () {
      let gcs = await RemainingShow({currentPage: this.currentPage, Order_Billnet: this.nodeData, pageCount: this.pageCount, searchData: this.momo.model})
      this.tableData = gcs.data.info
      this.total = gcs.data.count
    },
    async customSetting () {
      let customData = await SettlementCustom({Cus_Type: 'shyltj'})
    ...
    async customGo () {
      ...
      let ct = await SettlementCusadd({Cus_Name: ctData, Cus_Type: 'shyltj'})
      if (ct.data.result === '1') {
        this.$message({
          showClose: true,
      ...
      columns.forEach((column, index) => {
        ...
        if (!values.every(value => isNaN(value))) {
          sums[index] = values.reduce((prev, curr) => {
            ...
            if (!isNaN(value) & ((columns[index].label === '始发中转费') | (columns[index].label === '件数(件)') | (columns[index].label === '运费') | (columns[index].label === '货款') | (columns[index].label === '回扣'))) {
