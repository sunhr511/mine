      ...
      <el-col :span="20" ><br>
        <el-form :inline="true" :model="momo.model" ref="momo.model" :rules="rules" label-position="right" >
          <el-form-item  v-for="(item, index) in momo.data" :key="index" :label="item.label" :prop="item.props"  label-width="100px" >
            ...
            <el-input size="small" v-model="momo.model[item.props]" v-if="item.input" :placeholder="item.placeholder" style="width:150px;" class="inputStyle"></el-input>
          </el-form-item>
...
import {LoandetailShow} from '../../api/baobiaofenxi/fkmxtj.js'
...
export default {
  ...
  methods: {
    ...
    async show () {
      let gcs = await LoandetailShow({currentPage: this.currentPage, Order_Billnet: this.nodeData, pageCount: this.pageCount, searchData: this.momo.model})
      if (gcs.data.info) {
    ...
    async customSetting () {
      let customData = await SettlementCustom({Cus_Type: 'fkmxtj'})
    ...
    async customGo () {
      ...
      let ct = await SettlementCusadd({Cus_Name: ctData, Cus_Type: 'fkmxtj'})
      if (ct.data.result === '1') {
        this.$message({
          showClose: true,
