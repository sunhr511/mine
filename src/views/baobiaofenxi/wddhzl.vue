...
import {RetentionShow} from '../../api/baobiaofenxi/wddhzl.js'
...
export default {
  ...
  methods: {
    ...
    async show () {
      let gcs = await RetentionShow({currentPage: this.currentPage, Order_Billnet: this.nodeData, pageCount: this.pageCount, searchData: this.momo.model})
      this.tableData = gcs.data.info
      this.total = gcs.data.count
    },
    async customSetting () {
      let customData = await SettlementCustom({Cus_Type: 'wddzl'})
    ...
    async customGo () {
      ...
      let ct = await SettlementCusadd({Cus_Name: ctData, Cus_Type: 'wddzl'})
      if (ct.data.result === '1') {
        this.$message({
          showClose: true,
