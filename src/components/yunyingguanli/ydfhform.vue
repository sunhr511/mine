...
<br>
    ...
    <div style="display:flex;justify-content: space-between">
      <el-form-item :label="item.label" :prop="item.prop" label-width="85px" v-for="(item, index) in all.fifth.data" :key="index">
        <el-input v-model="formInfo[item.prop]" v-if="item.input" :change="aaa()" :placeholder="item.placeholder" ></el-input>
    ...
    <div style="margin-top:6px;display:flex;height:30px" v-if="!printTF">
      ...
      <el-form-item label="数量" label-width="90px">
        <el-input size="small" placeholder="请输入数量默认为件数" v-model="formInfo.num1"></el-input>
      </el-form-item>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
export default {
  ...
  created () {
    ...
    this.resetForm()
    // this.formInfo.num1 = this.formInfo.Order_Num
  },
  data () {
    return {
      checked: false,
  ...
  methods: {
    ...
    submitForm (formInfo) {
      ...
      this.$refs[formInfo].validate((valid) => {
        if (valid) {
          // alert('submit!')
          console.log(this.formInfo.num1)
          this.$emit('submitForm', this.formInfo, this.all, this.checked1, this.checked2, this.formInfo.num1)
        } else {
